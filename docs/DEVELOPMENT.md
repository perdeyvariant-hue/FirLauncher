...
- **Запуск по ярлыку без лаунчера.** С `--launch` окно создаётся скрытым (`visible: false` в
  `tauri.conf.json`, показывает его `setup`), фронтенд рисует не оболочку, а карточку
  `features/launch/DirectLaunch.tsx` размером 420×250 и показывает окно сам. По `game://started`
